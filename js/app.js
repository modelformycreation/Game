...
const gameEngine = {
    ...
    playPatternOracle() {
        ...
        function generateRound() {
            ...
            // Generate options - make wrong answers close to the correct one
            const wrongAnswers = [];

            // Add various wrong answers: slightly off, way off, etc.
            wrongAnswers.push(answer + utils.randomInt(10, 50));
            wrongAnswers.push(answer - utils.randomInt(10, 50));
            wrongAnswers.push(answer + utils.randomInt(51, 200));

            // Shuffle and pick 3 wrong answers
            const shuffledWrong = utils.shuffle(wrongAnswers).slice(0, 3);

            // Create options array with correct answer and 3 wrong ones
            options = [answer, ...shuffledWrong];

            // Make sure all options are unique and positive
            options = options.filter(v => v > 0).filter((v, i, a) => a.indexOf(v) === i);

            // If we don't have 4 unique options, add more
            while (options.length < 4) {
                options.push(answer + (options.length * 100));
            }
    ...
    selectPatternOption(selected) {
        ...
        optionEls.forEach(el => {
            ...
            el.onclick = null; // Remove onclick to prevent double-triggering
            const elValue = parseInt(el.textContent);
            if (elValue === answer) {
                el.classList.add('correct');
            } else if (elValue === selected) {
        ...
        area.innerHTML = `
            <div class="game-start">
                <div class="game-start-header">
                    ...
                    <h2 class="game-start-title">Color Blitz</h2>
                    <p class="game-start-description">Press the button matching the COLOR, not the word! Watch out for the trick!</p>
                </div>
                <div class="game-start-info">
                    <div class="info-card">
                        <i class="ph ph-target info-icon"></i>
                    ...
                    <div class="info-card">
                        <i class="ph ph-lightning info-icon"></i>
                        <div class="info-value">Fast!</div>
                        <div class="info-label">Speed</div>
                    </div>
                </div>
                <button class="btn btn-primary btn-lg" onclick="gameEngine.playColorBlitz()">
                    <i class="ph ph-play"></i>
                    Start Game
                </button>
            </div>
    ...
    playColorBlitz() {
        ...
        area.innerHTML = `
            <div class="color-blitz-area">
                <div class="color-prompt">
                    <div class="color-word" id="color-word" style="color: ${colors[0]}">${colorNames[0]}</div>
                </div>
                <div class="color-buttons">
                    <button class="color-btn red" onclick="gameEngine.checkColorBlitz(0)">RED</button>
                    <button class="color-btn blue" onclick="gameEngine.checkColorBlitz(1)">BLUE</button>
                    <button class="color-btn green" onclick="gameEngine.checkColorBlitz(2)">GREEN</button>
                    <button class="color-btn yellow" onclick="gameEngine.checkColorBlitz(3)">YELLOW</button>
                </div>
                <div class="color-score">
                    <div class="color-score-value" id="color-correct">${correct}</div>
                    <div class="color-score-label">Correct</div>
                    <div id="color-round" style="margin-top: 10px; color: var(--text-muted);">${currentRound}/${maxRounds}</div>
                </div>
            </div>
        `;
        ...
        area.innerHTML += `<div id="color-feedback" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); font-size: 3rem; display: none;"></div>`;
        ...
        function generateRound() {
            ...
            currentRound++;
            document.getElementById('color-round').textContent = `${currentRound}/${maxRounds}`;

            // Pick a random color
            currentColor = utils.randomInt(0, 3);
            const wordEl = document.getElementById('color-word');
            wordEl.textContent = colorNames[currentColor];
            wordEl.style.color = colors[currentColor];
        ...
        window.colorBlitzCorrect = 0;
    },

    checkColorBlitz(selected) {
        const maxRounds = 20;
        const correct = parseInt(document.getElementById('color-correct').textContent);

        // Count must match - the word color must match button
        const wordColor = document.getElementById('color-word').textContent;
        const correctAnswer = ['RED', 'BLUE', 'GREEN', 'YELLOW'][selected];

        let newCorrect = correct;
        if (wordColor === correctAnswer) {
            newCorrect++;
            document.getElementById('color-correct').textContent = newCorrect;
            document.getElementById('current-score').textContent = newCorrect;
        }

        setTimeout(() => {
            const currentRound = parseInt(document.getElementById('color-round').textContent.split('/')[0]);
            if (currentRound < maxRounds) {
                // Generate next round
                const colors = ['red', 'blue', 'green', 'yellow'];
                const colorNames = ['RED', 'BLUE', 'GREEN', 'YELLOW'];
                const nextColor = utils.randomInt(0, 3);
                const wordEl = document.getElementById('color-word');
                wordEl.textContent = colorNames[nextColor];
                wordEl.style.color = colors[nextColor];
                document.getElementById('color-round').textContent = `${currentRound + 1}/${maxRounds}`;
            } else {
                // Game over
                const finalScore = parseInt(document.getElementById('color-correct').textContent);
                const results = {
                    gameId: 'color-blitz',
                    score: finalScore,
                    maxScore: maxRounds,
                    accuracy: Math.round((finalScore / maxRounds) * 100),
                    level: gameEngine.level,
                    timestamp: Date.now()
                };
                gameEngine.showResults(results);
    ...
    playSequenceBreak() {
        ...
        area.innerHTML = `
            <div class="sequence-area">
                <div class="sequence-grid" id="sequence-grid" style="grid-template-columns: repeat(${gridSize}, 1fr);">
                    ${shuffled.map(num => `<div class="sequence-cell" data-num="${num}" onclick="gameEngine.clickSequence(${num})">${num}</div>`).join('')}
                </div>
                <div class="sequence-progress">
                    <span class="sequence-current">Next: <strong id="next-number">1</strong></span>
                    <span class="sequence-time" id="sequence-time">00:00</span>
                </div>
                <div style="margin-top: 20px; color: var(--text-muted);">Click numbers 1 to 25 in order!</div>
            </div>
        `;

        gameEngine.startTime = startTime;
        gameEngine.timer = setInterval(() => {
            const elapsed = Math.floor((Date.now() - gameEngine.startTime) / 1000);
            document.getElementById('sequence-time').textContent = utils.formatTime(elapsed);
        }, 1000);
    },

    clickSequence(num) {
        const nextExpected = parseInt(document.getElementById('next-number').textContent);
        const cell = document.querySelector(`[data-num="${num}"]`);

        if (num === nextExpected) {
            cell.classList.add('clicked');
            cell.style.pointerEvents = 'none';
            document.getElementById('next-number').textContent = num + 1;
            ...
            if (num === 25) {
                ...
                const time = (Date.now() - gameEngine.startTime) / 1000;
                const results = {
                    gameId: 'sequence-break',
                    score: Math.round(2500 - (time * 10)),
                    maxScore: 2500,
                    time: time,
                    accuracy: 100,
                    level: gameEngine.level,
    ...
    playCardCascade() {
        ...
        area.innerHTML = `
            <div class="card-cascade-area">
                <div class="card-cascade-grid" id="card-grid" style="grid-template-columns: repeat(4, 1fr);">
                    ${shuffled.map((card, i) => `
                        <div class="game-card" data-index="${i}" data-id="${card.id}" onclick="gameEngine.flipCard(${i}, ${card.id})">
                ...
                <div style="margin-top: 20px; color: var(--text-secondary);">
                    Matches: <span id="cascade-matches">0</span>/${pairs} | Moves: <span id="cascade-moves">0</span>
                </div>
            </div>
        `;

        window.cascadeFlipped = [];
        window.cascadeMatched = 0;
        window.cascadeLocked = false;
    },

    flipCard(index, id) {
        if (window.cascadeLocked) return;

        const card = document.querySelectorAll('.game-card')[index];
        if (card.classList.contains('flipped') || card.classList.contains('matched')) return;

        card.classList.add('flipped');
        window.cascadeFlipped.push({ index, id, card });

        if (window.cascadeFlipped.length === 2) {
            window.cascadeLocked = true;
            const [first, second] = window.cascadeFlipped;

            document.getElementById('cascade-moves').textContent = parseInt(document.getElementById('cascade-moves').textContent) + 1;

            if (first.id === second.id) {
                // Match!
                first.card.classList.add('matched');
                second.card.classList.add('matched');
                window.cascadeMatched++;
                document.getElementById('cascade-matches').textContent = window.cascadeMatched;
                document.getElementById('current-score').textContent = window.cascadeMatched;

                window.cascadeFlipped = [];
                window.cascadeLocked = false;

                if (window.cascadeMatched === 6) {
                    // Won!
                    const results = {
                        gameId: 'card-cascade',
                        score: window.cascadeMatched * 100,
                        maxScore: 600,
                        accuracy: 100,
                        level: gameEngine.level,
                        timestamp: Date.now()
    ...
    playEchoRecall() {
        ...
        area.innerHTML = `
            <div class="echo-recall-area">
                <div id="echo-status" style="font-size: 1.5rem; margin-bottom: 20px;">Watch the sequence...</div>
                <div class="echo-sequence" id="echo-sequence"></div>
                <div class="echo-input" id="echo-input" style="margin-top: 20px; opacity: 0.3; pointer-events: none;">
                    ${symbols.slice(0, 4).map((sym, i) => `<button class="echo-btn" onclick="gameEngine.clickEcho(${i})" style="background: var(--bg-tertiary);">${sym}</button>`).join('')}
                </div>
                <div id="echo-progress" style="margin-top: 20px;">Round: 0/${sequenceLength}</div>
            </div>
        `;
        ...
        function showSequence(index) {
            if (index >= sequence.length) {
                phase = 'playing';
                document.getElementById('echo-status').textContent = 'Your turn! Repeat the sequence';
                document.getElementById('echo-input').style.opacity = '1';
                document.getElementById('echo-input').style.pointerEvents = 'auto';
                return;
            }

            document.getElementById('echo-progress').textContent = `Watch: ${index + 1}/${sequence.length}`;

            const echoSeq = document.getElementById('echo-sequence');
            const symbol = symbols[sequence[index]];
            echoSeq.innerHTML = `<div class="echo-item active" style="width: 80px; height: 80px; font-size: 2.5rem; background: var(--gradient-primary);">${symbol}</div>`;
        ...
        window.echoSequence = sequence;
        window.echoPlayerSequence = [];
        window.echoCorrect = 0;
    },

    clickEcho(index) {
        const symbols = ['🔴', '🔵', '🟢', '🟡'];
        const echoSeq = document.getElementById('echo-sequence');
        echoSeq.innerHTML = `<div class="echo-item active" style="width: 80px; height: 80px; font-size: 2.5rem;">${symbols[index]}</div>`;

        window.echoPlayerSequence.push(index);

        const seq = window.echoSequence;
        const player = window.echoPlayerSequence;
        const currentIndex = player.length - 1;

        if (player[currentIndex] !== seq[currentIndex]) {
            // Wrong!
            echoSeq.innerHTML = `<div class="echo-item" style="width: 80px; height: 80px; font-size: 2.5rem; background: rgba(239, 68, 68, 0.2); border-color: var(--accent-red);">❌</div>`;

            const results = {
                gameId: 'echo-recall',
                score: window.echoCorrect,
                maxScore: seq.length,
                accuracy: Math.round((window.echoCorrect / seq.length) * 100),
                level: gameEngine.level,
                timestamp: Date.now()
            };
            setTimeout(() => gameEngine.showResults(results), 1000);
            return;
        }

        window.echoCorrect++;
        document.getElementById('current-score').textContent = window.echoCorrect;
        document.getElementById('echo-progress').textContent = `Round: ${player.length}/${seq.length}`;
        ...
        if (player.length === seq.length) {
            ...
            setTimeout(() => gameEngine.showResults(results), 500);
        }
    },
    ...
    playFocusZone() {
        ...
        area.innerHTML = `
            <div class="focus-zone-area">
                <div class="focus-task">
                    <div class="focus-task-prompt">What is 7 + 8?</div>
                    <div style="display: flex; gap: 10px; justify-content: center; margin-top: 15px;">
                        <button class="btn btn-secondary" onclick="gameEngine.answerFocus(true)">15</button>
                        <button class="btn btn-secondary" onclick="gameEngine.answerFocus(false)">14</button>
                        <button class="btn btn-secondary" onclick="gameEngine.answerFocus(false)">16</button>
                    </div>
                </div>
                <div style="margin-top: 20px;">
                    <span style="color: var(--accent-cyan); font-size: 1.5rem;" id="focus-score">Score: 0</span>
                    <span style="margin-left: 20px; color: var(--text-muted);">Round: <span id="focus-round">0</span>/${maxRounds}</span>
                </div>
                <div id="focus-distraction" style="position: absolute; display: none; background: var(--accent-amber); color: black; padding: 10px 20px; border-radius: 8px; cursor: pointer;">
                    IGNORE THIS!
                </div>
            </div>
        `;
        ...
        function generateQuestion() {
            ...
            round++;
            document.getElementById('focus-round').textContent = round;

            const a = utils.randomInt(1, 20);
            const b = utils.randomInt(1, 20);
            const correct = a + b;
            const wrong = correct + utils.randomInt(1, 3) * (Math.random() > 0.5 ? 1 : -1);

            document.querySelector('.focus-task-prompt').textContent = `What is ${a} + ${b}?`;
            const buttons = document.querySelectorAll('.focus-task button');
            const answers = utils.shuffle([correct, wrong, wrong + 1]);
            buttons.forEach((btn, i) => {
                btn.textContent = answers[i];
                btn.onclick = () => gameEngine.answerFocus(answers[i] === correct);
            });
        }
        ...
        function startDistractions() {
            distractionInterval = setInterval(() => {
                if (Math.random() > 0.5) {
                    const dist = document.getElementById('focus-distraction');
                    dist.style.display = 'block';
                    dist.style.left = Math.random() * 60 + 20 + '%';
                    dist.style.top = Math.random() * 60 + 20 + '%';
                    setTimeout(() => dist.style.display = 'none', 1500);
                }
            }, 2000);
        }

        window.answerFocus = (correct) => {
            if (correct) {
                window.focusScore += 10;
                document.getElementById('focus-score').textContent = `Score: ${window.focusScore}`;
                document.getElementById('current-score').textContent = window.focusScore;
            }
        ...
        function endGame() {
            ...
            const results = {
                gameId: 'focus-zone',
                score: window.focusScore,
                maxScore: maxRounds * 10,
                accuracy: Math.round((window.focusScore / (maxRounds * 10)) * 100),
                level: gameEngine.level,
                timestamp: Date.now()
            };
            gameEngine.showResults(results);
    ...
    renderGenericGameStart(area) {
        ...
        // Redirect to actual game or show message
        const actualGames = ['neural-snap', 'reaction-test', 'grid-lock', 'pattern-oracle', 'mental-math', 'dual-nback'];

        if (actualGames.includes(gameId)) {
        ...
        area.innerHTML = `
            <div class="game-start">
                ...
                <div class="game-start-info">
                    ...
                    <div class="info-card">
                        <i class="ph ph-star info-icon"></i>
                        <div class="info-value">${this.currentGame?.category || 'General'}</div>
                        <div class="info-label">Category</div>
                    </div>
                </div>
                <button class="btn btn-primary btn-lg" onclick="gameEngine.startLevel('${gameId}')">
                    <i class="ph ph-play"></i>
    ...
    startLevel(gameId) {
        ...
        switch(gameId) {
            case 'neural-snap':
                ...
                break;
            case 'reaction-test':
                this.playReactionTest();
                break;
            case 'grid-lock':
                this.playGridLock();
                break;
            case 'pattern-oracle':
                this.playPatternOracle();
                break;
            case 'mental-math':
                this.playMentalMath();
                break;
            ...
            case 'color-blitz':
                ...
                break;
            case 'sequence-break':
                this.playSequenceBreak();
                break;
            case 'card-cascade':
                this.playCardCascade();
                break;
            case 'echo-recall':
                this.playEchoRecall();
                break;
            case 'focus-zone':
                ...
                break;
            default:
                // Show a simple reaction test as fallback
                this.playReactionTest();
        }
    },
    ...
    showResults(results) {
        ...
        const performance = results.maxScore ? results.score / results.maxScore : (results.accuracy || 0) / 100;
        ...
        if (isWin && this.level < this.currentGame.levels) {
            difficultyText = '<span class="badge badge-emerald">Level Up! → Level ' + nextLevel + '</span>';
        } else if (!isWin && this.level > 1) {
            difficultyText = '<span class="badge badge-amber">Level Adjusted → Level ' + nextLevel + '</span>';
        }
